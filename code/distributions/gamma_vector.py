...
# TN draws
def gamma_vector_draw(alphas,betas):
    draws = []
    for (alpha,beta) in zip(alphas,betas):
        d = inverted_beta.gamma_draw(alpha,beta)
        d = d if (d >= 0. and d != numpy.inf and d != -numpy.inf and not numpy.isnan(d)) else 0.
        draws.append(d)   
    return draws           
       
# IB expectation    
def gamma_vector_expectation(alphas,betas):
    v = numpy.divide(alphas, betas)
    return numpy.where( (v < 0.0 ) & (v == -numpy.inf) & (numpy.isnan(v)) , 0., v)
    

# TN variance
def gamma_vector_variance(alphas,betas):
    numerator = alphas
    denomenator = numpy.square(betas)
